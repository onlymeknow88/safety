...
use App\Models\MasterData\ReportType;
...
class ReportTypeController extends Controller
{
    ...
    {
        $validator = Validator::make($request->all(), ['code' => 'required|string|max:255', 'is_active' => 'boolean']);
        if ($validator->fails()) return SafetyResponse::error($validator->errors(), 'Validasi Gagal', 422);
        $data = ReportType::create($request->all());
