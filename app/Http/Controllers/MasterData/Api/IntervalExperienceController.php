...
class IntervalExperienceController extends Controller
{
    public function index(Request $request)
    {
        $search = $request->search;
        $load = $request->load ?? 10;
        $query = IntervalExperience::query();
    ...
    public function store(Request $request)
    {
        ...
        $data = IntervalExperience::create($request->all());
        return SafetyResponse::success($data, 'Berhasil menambahkan data');
    }

    public function show($id)
    {
        $data = IntervalExperience::find($id);
