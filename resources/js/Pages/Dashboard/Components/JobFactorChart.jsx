...
export default function JobFactorChart({ data = [], isDarkMode, palette = [] }) {
    ...
    // Limit to top 7 job factors
    const displayData = data.slice(0, 7);

    const chartColor = '#8b5cf6'; // Uniform Purple for Job Factor
        ...
        <Card
            title={
                <span style={{ fontWeight: 800, fontSize: 14, color: labelColor, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                    <ToolOutlined style={{ marginRight: 8, color: '#8b5cf6' }} />
        ...
        >
            <div style={{ height: 320, position: 'relative' }}>
                {data.length > 0 ? (
                    ...
                    <Empty description="Belum ada data faktor pekerjaan" style={{ paddingTop: 80 }} />
                )}
            </div>
        </Card>
    );
