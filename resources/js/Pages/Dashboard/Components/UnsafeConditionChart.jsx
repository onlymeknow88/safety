...
export default function UnsafeConditionChart({ data = [], isDarkMode, palette = [] }) {
    ...
    // Limit to top 7 conditions
    const displayData = data.slice(0, 7);

    const chartColor = '#3b82f6'; // Uniform Blue for USC
    ...
    const options = {
        ...
        layout: {
            padding: {
                bottom: 20, // Reduced bottom padding since positive rotation is more compact
                top: 15
            }
        },
        plugins: {
        ...
        <Card
            title={
                <span style={{ fontWeight: 800, fontSize: 14, color: labelColor, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                    <LineChartOutlined style={{ marginRight: 8, color: '#3b82f6' }} />
        ...
        >
            <div style={{ height: 320, position: 'relative' }}>
                {data.length > 0 ? (
                    ...
                    <Empty description="Belum ada data kondisi tidak aman" style={{ paddingTop: 80 }} />
                )}
            </div>
        </Card>
    );
