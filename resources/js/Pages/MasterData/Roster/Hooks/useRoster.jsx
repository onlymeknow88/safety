...
export default function useRoster() {
    ...
    const [postRequest] = usePost("roster");
    const [putRequest] = usePut("roster");
    const [deleteRequest] = useDelete("roster");
    ...
    const fetchItems = useCallback(async (params = {}) => {
        ...
        } catch (error) { notification.error({ message: "Gagal mengambil data roster" }); }
    ...
    const columns = useMemo(() => [
        { header: "POLA ROSTER", accessorKey: "pattern", cell: ({ row }) => <span style={{ fontWeight: 600 }}>{row.original.pattern}</span> },
