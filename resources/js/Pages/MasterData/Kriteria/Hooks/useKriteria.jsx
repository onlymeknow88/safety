...
export default function useKriteria() {
    const { notification } = App.useApp();
    const [getRequest, getFeedback] = useGet();
    const [postRequest] = usePost("kriteria");
    const [putRequest] = usePut("kriteria");
    const [deleteRequest] = useDelete("kriteria");
    ...
    const columns = useMemo(() => [
        { header: "NAMA KRITERIA", accessorKey: "name", cell: ({ row }) => <span style={{ fontWeight: 600 }}>{row.original.name}</span> },
