...
export default function useIntervalTime() {
    const { notification } = App.useApp();
    const [getRequest, getFeedback] = useGet();
    const [postRequest] = usePost("interval-time");
    const [putRequest] = usePut("interval-time");
    const [deleteRequest] = useDelete("interval-time");
    ...
    const fetchItems = useCallback(async (params = {}) => {
        ...
        } catch (error) { notification.error({ message: "Gagal mengambil data" }); }
    ...
    const columns = useMemo(() => [
        { header: "LABEL WAKTU", accessorKey: "label", cell: ({ row }) => <span style={{ fontWeight: 600 }}>{row.original.label}</span> },
    ...
    return { table, loading: getFeedback.loading, searchText, handleSearchChange, isModalVisible, setIsModalVisible, handleAdd: () => { setEditingItem(null); setIsModalVisible(true); }, handleEdit: (r) => { setEditingItem(r); setIsModalVisible(true); }, handleOk, isDeleteModalVisible, setIsDeleteModalVisible, handleConfirmDelete, itemToDelete, editingItem, totalRows, fetchItems };
}
