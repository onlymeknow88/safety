...
export default function SourceModal({ visible, onCancel, onFinish, loading, initialValues }) {
    ...
    return (
        <Modal title={initialValues ? "Edit Source" : "Tambah Source Baru"} open={visible} onCancel={onCancel} footer={null} destroyOnHidden>
            <Form form={form} layout="vertical" onFinish={onFinish} initialValues={{ is_active: true }}>
                <Form.Item label="Kode" name="code" rules={[{ required: true, message: "Masukan kode!" }]}>
                    <Input placeholder="Contoh: SRC-01" />
                </Form.Item>
                <Form.Item label="Deskripsi" name="description" rules={[{ required: true, message: "Masukan deskripsi!" }]}>
                    <Input.TextArea rows={3} placeholder="Masukan deskripsi sumber" />
                </Form.Item>
