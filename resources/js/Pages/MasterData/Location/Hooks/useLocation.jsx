...
export default function useLocation() {
    ...
    const [postRequest] = usePost("location");
    const [putRequest] = usePut("location");
    const [deleteRequest] = useDelete("location");
    ...
    const columns = useMemo(() => [
        { header: "CCOW", accessorKey: "ccow.name", cell: ({ row }) => <Tag color="blue">{row.original.ccow?.name || "-"}</Tag> },
        { header: "NAMA LOKASI", accessorKey: "name", cell: ({ row }) => <span style={{ fontWeight: 600 }}>{row.original.name}</span> },
