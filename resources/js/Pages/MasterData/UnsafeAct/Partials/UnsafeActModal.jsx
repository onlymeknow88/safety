...
export default function UnsafeActModal({ visible, onCancel, onFinish, loading, initialValues }) {
    ...
    return (
        <Modal title={initialValues ? "Edit Unsafe Act" : "Tambah Unsafe Act"} open={visible} onCancel={onCancel} footer={null} destroyOnHidden>
            <Form form={form} layout="vertical" onFinish={onFinish} initialValues={{ is_active: true }}>
                <Form.Item label="Kode" name="code" rules={[{ required: true, message: "Masukan kode!" }]}>
                    <Input placeholder="Contoh: UA-01" />
                </Form.Item>
                <Form.Item label="Deskripsi" name="description" rules={[{ required: true, message: "Masukan deskripsi!" }]}>
                    <Input.TextArea rows={3} placeholder="Masukan deskripsi tindakan tidak aman" />
                </Form.Item>
