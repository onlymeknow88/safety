...
export default function LocationGeneralTable({ table, loading, totalRows, isDarkMode }) {
    ...
    return (
        <div style={{ background: isDarkMode ? "#1e293b" : "#fff", borderRadius: "20px", padding: "1px", border: `1px solid ${isDarkMode ? '#334155' : '#e2e8f0'}`,
            boxShadow: isDarkMode ? "none" : "0 4px 24px rgba(0,0,0,0.04)", overflow: "hidden" }}>
            <Table rowKey="id" columns={antdColumns} dataSource={table.getRowModel().rows.map(r => r.original)} loading={loading} pagination={false} scroll={{ x: 600 }} className={`custom-antd-table ${isDarkMode ? 'dark-mode' : ''}`} style={{ borderRadius: "20px" }} />
            <div style={{ padding: "16px 24px", background: isDarkMode ? '#1e293b' : '#f8fafc',
                borderTop: `1px solid ${isDarkMode ? '#334155' : '#e2e8f0'}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <Space size="middle">
            ...
            <style>{`.custom-antd-table .ant-table { background: transparent !important; } .custom-antd-table .ant-table-thead > tr > th { background: ${isDarkMode ? "#1e293b" : "#f8fafc"} !important; color: ${isDarkMode ? "#cbd5e1" : "#475569"} !important; font-weight: 700 !important; border-bottom: 2px solid ${isDarkMode ? "#334155" : "#e2e8f0"} !important; padding: 16px !important; } .custom-antd-table .ant-table-tbody > tr > td { padding: 16px !important; border-bottom: 1px solid ${isDarkMode ? "#334155" : "#f1f5f9"} !important; transition: all 0.2s; } .custom-antd-table .ant-table-tbody > tr:hover > td { background: ${isDarkMode ? "#334155" : "#f0f9ff"} !important; }`}</style>
        </div>
    );
}
