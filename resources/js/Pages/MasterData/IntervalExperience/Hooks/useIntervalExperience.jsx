...
export default function useIntervalExperience() {
    const { notification } = App.useApp();
    const [getRequest, getFeedback] = useGet();
    const [postRequest] = usePost("interval-experience");
    const [putRequest] = usePut("interval-experience");
    const [deleteRequest] = useDelete("interval-experience");
    ...
    const columns = useMemo(() => [
        { header: "LABEL PENGALAMAN", accessorKey: "label", cell: ({ row }) => <span style={{ fontWeight: 600 }}>{row.original.label}</span> },
