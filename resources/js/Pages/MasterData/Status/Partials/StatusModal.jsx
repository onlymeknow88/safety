...
export default function StatusModal({ visible, onCancel, onFinish, loading, initialValues }) {
    ...
    return (
        <Modal title={initialValues ? "Edit Status" : "Tambah Status"} open={visible} onCancel={onCancel} footer={null} destroyOnHidden>
            <Form form={form} layout="vertical" onFinish={onFinish} initialValues={{ is_active: true }}>
                <Form.Item label="Nama Status" name="name" rules={[{ required: true, message: "Masukan nama status!" }]}>
                    <Input placeholder="Contoh: Terkirim / Selesai" />
                </Form.Item>
