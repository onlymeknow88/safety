...
export default function useStatus() {
    ...
    const [postRequest] = usePost("status");
    const [putRequest] = usePut("status");
    const [deleteRequest] = useDelete("status");
    ...
    const fetchItems = useCallback(async (params = {}) => {
        ...
        } catch (error) { notification.error({ message: "Gagal mengambil data status" }); }
    ...
    const columns = useMemo(() => [
        { header: "NAMA STATUS", accessorKey: "name", cell: ({ row }) => <span style={{ fontWeight: 600 }}>{row.original.name}</span> },
        { header: "STATUS AKTIF", accessorKey: "is_active", cell: ({ row }) => <Tag color={row.original.is_active ? "green" : "red"}>{row.original.is_active ? "ACTIVE" : "INACTIVE"}</Tag> },
