...
export default function UnsafeConditionHeader({ searchText, onSearchChange, onAddClick, isDarkMode, table }) {
    ...
    return (
        <div style={{ marginBottom: 24, display: "flex", justifyContent: "space-between", alignItems: "center", background: isDarkMode ? "#1f1f1f" : "#fff", padding: "16px 24px", borderRadius: 16 }}>
            <Space size={16}>
                <Input placeholder="Cari Unsafe Condition..." prefix={<SearchOutlined style={{ color: "#bfbfbf" }} />} value={searchText} onChange={onSearchChange} style={{ width: 320, borderRadius: 10, background: isDarkMode ? "#141414" : "#f5f5f5", border: "none", height: 40 }} />
            ...
            <Button type="primary" icon={<PlusOutlined />} onClick={onAddClick} style={{ height: 40, borderRadius: 10, fontWeight: 600 }}>Tambah Data</Button>
        </div>
    );
}
