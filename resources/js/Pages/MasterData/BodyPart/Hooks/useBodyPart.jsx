...
export default function useBodyPart() {
    const { notification } = App.useApp();
    const [getRequest, getFeedback] = useGet();
    const [postRequest] = usePost("body-part");
    const [putRequest] = usePut("body-part");
    const [deleteRequest] = useDelete("body-part");
    ...
    const columns = useMemo(() => [
        { header: "NAMA BAGIAN", accessorKey: "name", cell: ({ row }) => <span style={{ fontWeight: 600 }}>{row.original.name}</span> },
