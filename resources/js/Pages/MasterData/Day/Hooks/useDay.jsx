...
export default function useDay() {
    ...
    const [postRequest, postFeedback] = usePost("day");
    const [putRequest, putFeedback] = usePut("day");
    const [deleteRequest, deleteFeedback] = useDelete("day");
    ...
    const fetchItems = useCallback(async (params = {}) => {
        ...
        } catch (error) { notification.error({ message: "Gagal mengambil data Day" }); }
    ...
    const showDeleteModal = (record) => { setItemToDelete(record); setIsDeleteModalVisible(true); };

    const handleOk = async (values) => {
        try {
            const res = editingItem ? await putRequest(values, editingItem.id) : await postRequest(values);
            if (res.data?.meta?.status === 'success') {
                notification.success({ message: "Berhasil menyimpan data" });
    ...
    const columns = useMemo(() => [
        { header: "NAMA HARI", accessorKey: "name", cell: ({ row }) => <span style={{ fontWeight: 600 }}>{row.original.name}</span> },
    ...
    return { table, loading: getFeedback.loading || postFeedback.loading || putFeedback.loading || deleteFeedback.loading, searchText, handleSearchChange, isModalVisible, setIsModalVisible, handleAdd, handleEdit, handleOk, isDeleteModalVisible, setIsDeleteModalVisible, showDeleteModal, handleConfirmDelete, itemToDelete, editingItem, totalRows, fetchItems };
}
