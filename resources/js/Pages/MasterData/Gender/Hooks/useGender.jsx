...
export default function useGender() {
    ...
    const [postRequest] = usePost("gender");
    const [putRequest] = usePut("gender");
    const [deleteRequest] = useDelete("gender");
    ...
    const fetchItems = useCallback(async (params = {}) => {
        ...
        } catch (error) { notification.error({ message: "Gagal mengambil data Gender" }); }
    ...
    const columns = useMemo(() => [
        { header: "JENIS KELAMIN", accessorKey: "name", cell: ({ row }) => <span style={{ fontWeight: 600 }}>{row.original.name}</span> },
    ...
    return { table, loading: getFeedback.loading, searchText, handleSearchChange, isModalVisible, setIsModalVisible, handleAdd, handleEdit, handleOk, isDeleteModalVisible, setIsDeleteModalVisible, handleConfirmDelete, itemToDelete, editingItem, totalRows };
}
