...
export default function PersonalFactorModal({ visible, onCancel, onFinish, loading, initialValues }) {
    ...
    return (
        <Modal title={initialValues ? "Edit Personal Factor" : "Tambah Personal Factor"} open={visible} onCancel={onCancel} footer={null} destroyOnHidden>
            <Form form={form} layout="vertical" onFinish={onFinish} initialValues={{ is_active: true }}>
                <Form.Item label="Kode" name="code" rules={[{ required: true, message: "Masukan kode!" }]}>
                    <Input placeholder="Contoh: PF-01" />
                </Form.Item>
                <Form.Item label="Deskripsi" name="description" rules={[{ required: true, message: "Masukan deskripsi!" }]}>
                    <Input.TextArea rows={3} placeholder="Masukan deskripsi faktor personal" />
                </Form.Item>
