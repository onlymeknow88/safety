...
export default function useInjuryCondition() {
    const { notification } = App.useApp();
    const [getRequest, getFeedback] = useGet();
    const [postRequest] = usePost("injury-condition");
    const [putRequest] = usePut("injury-condition");
    const [deleteRequest] = useDelete("injury-condition");
    ...
    const columns = useMemo(() => [
        { header: "NAMA KONDISI", accessorKey: "name", cell: ({ row }) => <span style={{ fontWeight: 600 }}>{row.original.name}</span> },
