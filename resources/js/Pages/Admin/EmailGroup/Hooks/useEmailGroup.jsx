...
export default function useEmailGroup() {
    const { notification } = App.useApp();

    const [getRequest, getFeedback] = useGet();
    const [postRequest, postFeedback] = usePost("email-groups");
    const [putRequest, putFeedback] = usePut("email-groups");
    const [deleteRequest, deleteFeedback] = useDelete("email-groups");
    ...
    const fetchItems = useCallback(async (params = {}) => {
        try {
            ...
            notification.error({ message: "Gagal mengambil data Grup Email" });
        }
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            ...
            if (response.data?.meta?.status === 'success') {
                notification.success({ message: "Grup email berhasil dihapus" });
    ...
    const columns = useMemo(() => [
        {
            ...
            cell: ({ row }) => (
                <div>
                    ...
                    {row.original.description && <Text type="secondary" style={{ fontSize: 11 }}>{row.original.description}</Text>}
                </div>
            )
        },
        {
            ...
            cell: ({ row }) => (
                <Space wrap>
                    <Tag color="blue" style={{ borderRadius: 6, fontWeight: 700 }}>
                        {row.original.recipients?.length || 0} Emails
                    </Tag>
                    {row.original.recipients?.slice(0, 2).map((r, i) => (
                        <Tag key={i} style={{ borderRadius: 4, fontSize: 11 }}>{r.email}</Tag>
                    ))}
                    {row.original.recipients?.length > 2 && <Text type="secondary" style={{ fontSize: 10 }}>+{(row.original.recipients.length - 2)} more</Text>}
                </Space>
            )
        },
        {
